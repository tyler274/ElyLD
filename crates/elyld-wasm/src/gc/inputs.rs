        ...
        let all_globals = file
            ...
            .map(|global| {
                let init_expr_body = crate::wasm_writer::const_expr_body(&global.init_expr)
                    .ok_or_else(|| {
                        elyld_error::error!("Wasm global initializer is missing end opcode")
                    })?;
                Ok(OutputGlobal {
                    ty: global.ty,
            ...
            match *resolution {
                ...
                ImportResolution::LinkerDefined(known) => {
                    let index = indices.function_index(known).ok_or_else(|| {
                        elyld_error::error!("missing reserved Wasm function for {known:?}")
                    })?;
            ...
            match *resolution {
                ...
                ImportResolution::LinkerDefined(known) => {
                    let index = indices.global_index(known).ok_or_else(|| {
                        elyld_error::error!("missing reserved Wasm global for {known:?}")
                    })?;
